...
class Grid:
    ...
    def __init__(self, rows, cols):
        ...
        self.matrix[0][0].set_type(CellType.VERTEX)
        self.matrix[rows - 1][0].set_type(CellType.VERTEX)
        self.matrix[0][cols - 1].set_type(CellType.VERTEX)
        self.matrix[rows - 1][cols - 1].set_type(CellType.VERTEX)
        ...
        for i in range(1, cols - 1):
            ...
            self.matrix[0][i].set_type(CellType.SIDE)
            self.matrix[rows - 1][i].vibrate_factor = 0.5
            self.matrix[rows - 1][i].set_type(CellType.SIDE)
